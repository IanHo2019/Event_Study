...
gen Y0_pure = 4*treated - 0.15*treated*t	// add a systematic time trend to treated units
gen eps = sqrt(0.07) * rnormal()	// error term
...
forvalues p = 1/2 {
	graph twoway (line lb`p' ub`p' etime, lpattern(dash dash) lcolor(brown brown)) ///
		(connected treated b`p' etime, mc(cranberry navy) msize(medium large) msymbol(oh o) lc(cranberry navy) lwidth(thin medium) lpattern(dot solid)), ///
		xline(-0.5, lcolor(red) lp(solid)) ///
		yline(0 , lpattern(dash) lcolor(gs8)) ///
		title("`title`p''", position(11)) ///
		xtitle("Event Time") xlab(, nogrid) ylab(-1.5(0.5)2) ///
